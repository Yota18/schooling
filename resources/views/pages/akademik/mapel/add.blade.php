						...
						<div class="col-md-7">
							<div class="card">
                                <div class="card-header card-info">
                                    <div class="card-title">Form tambah Pelajaran</div>
                                </div>
								<div class="card-body">
                                    <form action="{{ route('akademik.mapel.store') }}" method="POST"  enctype="multipart/form-data">
										@csrf
                                        <div class="row">
                                            <div class="form-group col-md-6">
                                                <label for="pelajaran">Nama Pelajaran</label>
                                                <input type="text" class="form-control" id="pelajaran" name="pelajaran" placeholder="Nama Pelajaran" value="{{ old('pelajaran') }}" >
                                                @error('pelajaran') <span class="text-danger">{{ $message }}</span> @enderror
                                            </div>
                                            <div class="form-group col-md-6">
                                                <label for="code">Kode Pelajaran</label>
                                                <input type="text" class="form-control" id="code" name="code" placeholder="Kode Pelajaran" value="{{ old('code') }}" >
                                            ...
                                            <div class="form-group col-md-6">
                                                ...
                                                <select class="form-control" name="category_pelajaran" id="category_pelajaran" style="width: 100%" value="{{ old('category_pelajaran') }}">
                                                    ...
                                                    @foreach ($categories as $category)
                                                        <option value="{{ $category->id }}" {{ old('category_pelajaran') == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                                    @endforeach
                                        ...
                                        <div class="mt-2 float-right">
                                            ...
                                            <button type="submit" class="btn btn-primary btn-rounded ml-2">Tambah Pelajaran</button>
                                        </div>
                                    </form>
								</div>
							</div>
