                ...
                <div class="panel-header bg-primary-gradient">
                    <div class="page-inner py-5">
                        <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
                            <div>
                                <h2 class="text-white pb-2 fw-bold">Alumni </h2>
                                <h5 class="text-white op-7 mb-2">Kelola informasi Alumni.</h5>
                            </div>
                            <div class="ml-md-auto py-2 py-md-0">
                                {{-- <a href="{{ route('master.users.student.add') }}" class="btn btn-secondary btn-round"><i class="fa fa-plus-circle mr-2" aria-hidden="true"></i>Tambah Siswa</a> --}}
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
			table =  $('#basic-datatables').DataTable({
				...
				ajax: {
					url : '{!! route('akademik.users.alumni.data') !!}',
					type : 'POST',
					data: {_token:_token},
				},
				columns: [
					...
					{ data: 'user.name' },
					{
						...
						render: function(data, type, row) {
							return '<p class="text-uppercase" style="margin-top: 15px;">'+data+'</p>'; 
						}
					},
					{
						data: 'id',
		...
		function set_switch(id,isactive){
			$.ajax({
				...
				url: '{!! route('akademik.users.alumni.switch') !!}',
				dataType: 'JSON',
