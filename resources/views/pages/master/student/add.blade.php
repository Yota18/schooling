                    ...
                    <div class="row">
						<div class="col-md-7">
							<div class="card">
								...
								<div class="card-body">
                                    <form action="{{ route('master.users.student.store') }}" method="POST"  enctype="multipart/form-data">
										@csrf
                                        <div class="row">
                                            <div class="form-group col-md-6">
                                                <label for="nis">NIS</label>
                                                <input type="text" class="form-control" id="nis" name="nis" aria-describedby="nis" placeholder="nis" value="{{ old('nis') }}" >
                                                @error('nis') <span class="text-danger">{{ $message }}</span> @enderror
                                            </div>
                                            <div class="form-group col-md-6">
                                                <label for="nisn">NISN</label>
                                                <input type="text" class="form-control" id="nisn" name="nisn" aria-describedby="nisn" placeholder="nisn" value="{{ old('nisn') }}" >
                                            ...
                                            <div class="form-group col-md-6">
                                                ...
                                                <select class="form-control" name="class" id="class" style="width: 100%">
                                                    ...
                                                    @foreach ($classes as $class)
                                                        <option value="{{ $class->id }}" {{ old('class') == $class->id ? 'selected' : '' }}> {{ strtoupper($class->code)  }}</option>
                                                    @endforeach
                                            ...
                                            <div class="form-group col-md-6">
                                                ...
                                                <input type="text" class="form-control" id="generation" name="generation"  placeholder="Angkatan. contoh: 2017" value="{{ old('generation') }}"  >
                                        ...
                                        <div class="mt-2 float-right">
                                            <a type="button" href="{{ route('master.users.student.index') }}" class="btn btn-warning btn-rounded ml-2">Kembali</a>
                                            <button type="submit" class="btn btn-primary btn-rounded ml-2">Tambah Siswa</button>
                                        </div>
                                    </form>
								</div>
							</div>
